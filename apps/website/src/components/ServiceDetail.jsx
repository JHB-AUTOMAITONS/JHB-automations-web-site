...
import { buildRel } from "@jhb/shared/service-links";
import { smartImgAttrs, stripHeadingTags, ALIGN_TEXT, ALIGN_BLOCK, ALIGN_JUSTIFY, toAlign, richTextAlign } from "@jhb/shared/containers";
...
const WI_SEC_BG = { none: "", subtle: "sec-bg-subtle", gradient: "sec-bg-gradient", dark: "sec-bg-dark" };
const WI_COLS = { 1: "", 2: "sm:grid-cols-2", 3: "sm:grid-cols-2 lg:grid-cols-3" };
function wiBgClass(bg) {
  return bg === "subtle"
    ? "bg-base"
    : bg === "gradient"
...
const isRich = (s) => /<[a-z][\s\S]*?>/i.test(s || "");
